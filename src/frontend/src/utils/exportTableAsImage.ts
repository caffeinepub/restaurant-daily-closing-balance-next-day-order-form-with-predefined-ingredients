...
export function exportConcernTableAsImage(params: {
  ...
  ctx.fillText("Shri Hoshnagi F&B", canvasWidth / 2, 28);

  ctx.font = "bold 13px sans-serif";
  ctx.fillStyle = "#f3f4f6";
  ctx.fillText(`Order #${orderNo}  ·  ${restaurantName}`, canvasWidth / 2, 50);

  ctx.font = "12px sans-serif";
  ctx.fillStyle = "#d1d5db";
  ctx.fillText(`Balance Date: ${balanceDate}`, canvasWidth / 2, 70);
  ctx.fillText(`Categories: ${categories.join(", ")}`, canvasWidth / 2, 88);
  ctx.fillText(`Total Ingredients: ${totalIngredients}`, canvasWidth / 2, 106);
  ...
  ctx.textAlign = "center";
  ctx.fillText(
    `Generated by Shri Hoshnagi F&B · ${new Date().toLocaleString()}`,
    canvasWidth / 2,
